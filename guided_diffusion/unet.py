...
class EBMUNetModel(nn.Module):
    ...
    def __init__(
        ...
        if num_classes is not None:
            self.label_emb = nn.Embedding(num_classes + 1, context_dim)
    ...
    def forward(self, x, timesteps, y=None, return_logits = False, cls_mode = False, cls_cond=False):
        ...
        if cls_mode:
            ...
            with th.enable_grad():
                ...
                h = x.type(self.dtype)
                h = input_tensor = th.autograd.Variable(h, requires_grad=True)
                ...
                if self.training:
                    x_prime = th.autograd.grad(logits_logsumexp.sum(), [input_tensor], create_graph=True, retain_graph=True)[0]
                else:
