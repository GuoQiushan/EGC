...
class FrozenAutoencoderVQ(nn.Module):
    def __init__(self, ddconfig, embed_dim, pretrained_path, n_embed, scale_factor=0.18215, sane_index_shape=False, remap = None):
        super().__init__()
        print(f'Create autoencoder with n_embed={n_embed}')
        self.encoder = Encoder(**ddconfig)
        self.decoder = Decoder(**ddconfig)
    ...
    def decode(self, quant, force_not_quantize=False):
        quant = quant / self.scale_factor
    ...
    def forward(self, input, fn):
        ...
        elif fn == 'encode_quant':
            return self.encode_quant(input)

def get_model(pretrained_path, model_type='KL', stride=8, scale_factor=0.18215):
    if model_type == 'KL':
        if stride == '8':
            ddconfig = dict(
                double_z=True,
            ...
            return FrozenAutoencoderKL(ddconfig, 3, pretrained_path, scale_factor)
    else:
        if stride == '4':
            ddconfig = dict(
                double_z=False,
            ...
            return FrozenAutoencoderVQ(ddconfig, 3, pretrained_path, 8192, scale_factor)
            ...
            return FrozenAutoencoderVQ(ddconfig, 4, pretrained_path, 16384, scale_factor)
            ...
            return FrozenAutoencoderVQ(ddconfig, 4, pretrained_path, 256, scale_factor)
